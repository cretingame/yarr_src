...
int SpecController::writeDma(uint32_t off, uint32_t *data, size_t words) {
    ...
    if ( status == DMAIDLE || status == DMADONE || status == DMAABORTED) {
        ...
        volatile uint32_t irq_ack = this->read32(bar2, GNGPIO_INT_STATUS/4);
        (void) irq_ack;
...
int SpecController::progFpga(const void *data, size_t size) {
    ...
    this->mask32(bar2, GNGPIO_DIRECTION_MODE/4, 0xC000, 0);
    this->mask32(bar2, GNGPIO_OUTPUT_ENABLE/4, 0xC000, 0xC000);
    this->mask32(bar2, GNGPIO_OUTPUT_VALUE/4, 0xC000, 0x8000);

    // FCL_CLK_DIV -> 0x0 -> PCLK/2 (PCLK = 125MHz)
    this->write32(bar2, FCL_CLK_DIV/4, 0x1);
    // FCL_CTRL -> 0x40 -> Reset
    this->write32(bar2, FCL_CTRL/4, 0x40);
    // Check reset is high
    if (0x40 != this->read32(bar2, FCL_CTRL/4)) {
        std::cerr << __PRETTY_FUNCTION__ << " -> Error setting FCL_CTRL ... aborting!" << std::endl;
        return -1;
    }

    // FCL_CTRL -> 0x0
    this->write32(bar2, FCL_CTRL/4, 0x0);
   
    // FCL_IRQ -> 0x0 -> Clear pending IRQ
    volatile uint32_t foo = this->read32(bar2, FCL_IRQ/4);
    (void) foo;
    ...
    this->write32(bar2, FCL_CTRL/4, ctrl);

    // FCL_TIMER_CTRL -> 0x0
    // FCL_TIMER_0 -> 0x10
    // FCL_TIMER_1 -> 0x0
    this->write32(bar2, FCL_TIMER_CTRL/4, 0x0);
    this->write32(bar2, FCL_TIMER_0/4, 0x10);
    this->write32(bar2, FCL_TIMER_1/4, 0x0);
    
    // Enable the right lines
    // FCL_EN -> 0x17
    this->write32(bar2, FCL_EN/4, 0x17);
	...
	while(size32 > 0)
	{
		...
		while (read32(bar2, FCL_IRQ/4)  & (1<<5))
			;

        // Loop
		for (i = 0; size32 && i < 32; i++) {
			write32(bar2, FCL_FIFO/4, BitOps::unaligned_bitswap_le32(data32));
			data32++; size32--; wrote++;
		}
	}
    // FCL_CTRL -> 0x186 (last data written)
    this->write32(bar2, FCL_CTRL/4, 0x186);
#ifdef DEBUG
    ...
    uint32_t irq = read32(bar2, FCL_IRQ/4);
    uint32_t status = read32(bar2, FCL_STATUS/4);
...
uint32_t SpecController::readEeprom(uint8_t * buffer, uint32_t len) {
    ...
    usleep(1000);
    this->mask32(bar2, LB_CTL/4, 0x0, 0x10000);
    ...
    usleep(1000);
    this->read32(bar2, TWI_IRT_STATUS/4);
    ...
    //this->write32(bar2, TWI_DATA/4, offset & 0xFF);
    usleep(1000);
    this->write32(bar2, TWI_DATA/4, 0);
    ...
    //this->write32(bar2, TWI_ADDRESS/4, devAddr & 0x7F);
    usleep(1000);
    this->write32(bar2, TWI_ADDRESS/4, 0x56);
    ...
    for(j = 1000000000, tmp = 0; !(tmp & 0x1); j--, usleep(1000)) {
        tmp = this->read32(bar2, TWI_IRT_STATUS/4);
        if(tmp & 0xC) {
    ...
    usleep(1000);
    this->mask32(bar2, TWI_CTRL/4, 0x0, 0x1);
    ...
    for(k = 1, tmp = 0; len>0; ) {
        ...
        usleep(1000);
        this->write32(bar2, TWI_TR_SIZE/4, k);
        ...
        for(tmp = 0; k>0; k--, usleep(1000)) {
            tmp = this->read32(bar2, TWI_DATA/4);
...
uint32_t SpecController::writeEeprom(uint8_t * buffer, uint32_t len, uint32_t offs) {
    ...
    for(k = 0; k<len; k++) {
        ...
        usleep(5000);
        this->write32(bar2, TWI_DATA/4, (offs + k) & 0xFF);
        ...
        usleep(5000);
        this->write32(bar2, TWI_DATA/4, *(buffer + k));
